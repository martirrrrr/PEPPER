...
def speech_recognition(ip, port):
    ...
    try:
        ...
        timeout = 30  # Timeout di 30 secondi per il riconoscimento
        start_time = time.time()

        while not word_recognized:
            # Controlla se è passato il timeout
            if time.time() - start_time > timeout:
                print("[ERRORE] Timeout raggiunto. Nessuna parola riconosciuta.")
                text_to_speech.say("Non ho riconosciuto nulla. Riprovare più tardi.")
                return
        ...
        # Gestione delle azioni in base alla parola riconosciuta
        if recognized_word == vocabulary[0]:  # meditare
            ...
            return  # Chiusura del programma
        else:
            print("[ERRORE] Parola non prevista.")
            text_to_speech.say("Non ho capito bene. Riprova.")
            return
